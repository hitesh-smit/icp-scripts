...
: '
    ...
    README: 
            ...
            Example: ./icp-320-single-node.sh ce, will install ICP CE.
...
HOSTNAME_IP=" "
HOSTNAME=" "
ICP_LOCATION=/opt/icp-320
ICP_EE_FILE=ibm-cloud-private-ppc64le-3.2.0.tar.gz
...
network_setup() {
    ...
    sed -i '/search/ i nameserver 1.1.1.1' /etc/resolv.conf
...
case $1 in
     ce)
            if [ "$#" -ne 2 ]; then
                ...
                echo "Installing ICP 3.2.0 CE"
                basic_setup
                network_setup $2
                INCEPTION=ibmcom/icp-inception:3.2.0
     ...
     ee)
            if [ "$#" -ne 3 ]; then
                ...
                echo "Installing ICP 3.2.0 EE"
                echo $1 $2 $3
                ICP_EE_FILE_URL=$3
                INCEPTION=ibmcom/icp-inception-$(uname -m):3.2.0-ee
                basic_setup
                network_setup $2
